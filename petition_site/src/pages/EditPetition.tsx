...
const EditPetition = () => {
    ...
    React.useEffect(() => {
        ...
        axios.get("http://192.168.1.17:4941/api/v1/petitions/" + id + "/supporters")
        .then((res) => {
    ...
    const deleteTier = (Tid: number) => {
        axios.delete("http://192.168.1.17:4941/api/v1/petitions/" + id + "/supportTiers/" + Tid, {headers: {
    ...
    const editTier = (tier: SupportTier) => {
        axios.patch("http://192.168.1.17:4941/api/v1/petitions/" + id + "/supportTiers/" + tier.supportTierId, tier, {headers: {
            "X-Authorization": token
        }}).catch((error) => {
            setNetworkError(true)
            console.error(error)
    ...
    const addTier = (tier: SupportTier) => {
        axios.put("http://192.168.1.17:4941/api/v1/petitions/" + id + "/supportTiers", tier, {headers: {
            "X-Authorization": token
        }}).then((res) => {
      ...
      const submit = () => {
            if (validate()) {
                ...
                axios.put("http://192.168.1.17:4941/api/v1/petitions", data, {headers: {"X-Authorization": token} })
                .then((res) => {
                    setUpdatedOpen(true)
                }, (error) => {
                    setNetworkError(true)
    ...
    const imageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files) {
            axios.put("http://192.168.1.17:4941/api/v1/petitions/" + id + "/image", e.target.files[0], {headers: {
                "X-Authorization": token,
    ...
    return (
        <div>
            ...
            <img src={"http://192.168.1.17:4941/api/v1/petitions/" + id + "/image"} alt="Petition" style={{width: "200px", height: "200px"}}/>
            <Snackbar
                open={networkError}
